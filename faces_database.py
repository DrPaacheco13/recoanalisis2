...
class FacesDatabase:
    ...
    def dump_faces(self, image, desc, name, path = None):
        ...
        log.debug("Dumping image with label {} and path {} on disk.".format(
            label, filename))
        if osp.exists(filename):
            log.warning(
                "File with the same name already exists at {}. So it won't be stored.".format(self.fg_path))
    ...
    def add_item(self, desc, label):
        ...
        if not label:
            ...
            log.warning(
                "Trying to store an item without a label. Assigned label {}.".format(label))
        else:
