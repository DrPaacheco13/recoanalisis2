    ...
    def process(self, frame):
        ...
        if self.allow_grow and len(unknowns) > 0:
            for i in unknowns:
                ...
                numero = random.randint(111111111, 999999999)
                name = f'face_{numero}'
...
def draw_detections(frame, frame_processor, detections, output_transform):
    ...
    for roi, landmarks, identity in zip(*detections):
        ...
        cv2.rectangle(frame, (xmin, ymin), (xmin + textsize[0], ymin - textsize[1]), (255, 255, 255), 1)

        face_roi = frame[max(0, ymin - 15): min(ymax + 15, frame.shape[0] - 1),
                         max(0, xmin - 15): min(xmax + 15, frame.shape[1] - 1)]
        ...
        crop_image = crop(frame_copy, roi)
        name = text_label
        if identity.id != FaceIdentifier.UNKNOWN_ID:
            try:    
                connection = mysql.connector.connect(
                    host='localhost',
                    database='recoanalisis',
                    user='root',
                    password=''
                )
                ...
                if connection.is_connected():
                    ...
                    label = "{}-0.jpg".format(name)
                    path = osp.abspath('faces')
                    filename = osp.join(path, label)

                        # Assuming you have a 'images' table with columns 'filename', 'label', 'path', etc.
                    query = "INSERT INTO faces (filename, label, path, confidence, age, gender) VALUES (%s, %s, %s, %s, %s, %s)"
                    values = (filename, name, filename, str(confidence), age_label, gender_label)
                    cursor.execute(query, values)
                    connection.commit()

                    if (confidence > 0.90):
                        path2 = osp.abspath('faces_real')
                        filename2 = osp.join(path2, label)
                        query = "INSERT INTO faces_real (path, label, confidence, age, gender) VALUES (%s, %s, %s, %s, %s)"
                        values = (filename2, label, str(confidence), age_label, gender_label)
                        cursor.execute(query, values)
                        connection.commit()
                        cv2.imwrite(filename2, crop_image)
                        log.debug("Image registered in MySQL database.")

            except Error as e:
                log.error("Error while connecting to MySQL: {}".format(e))
...
def extract_bounding_boxes(yolo_results):
    rois = []
    # Extract bounding box information from YOLOv8 results
    for boxes in yolo_results:
        # print(boxes)
        # print('box')
        face_box = boxes.boxes.xyxy
        for box_info in face_box:
            x1, y1, x2, y2 = box_info[0], box_info[1], box_info[2], box_info[3]
            width, height = x2 - x1, y2 - y1
            rois.append((x1.item(), y1.item(), width.item(), height.item()))
