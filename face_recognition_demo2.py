...
gen_prototxt = "gender_deploy.prototxt"
gen_model = "gender_net.caffemodel"
...
gen_net = cv2.dnn.readNet(gen_model, gen_prototxt)
...
class FrameProcessor:
    ...
    def process(self, frame):
        ...
        results = model.predict(frame, show=False, conf=0.77)
     
        rois = []
      
        if results:
            rois = extract_bounding_boxes(results, frame)      
        ...
        face_identities, unknowns, all_list = self.face_identifier.infer((frame, rois, landmarks))
        ...
        if self.allow_grow and len(unknowns) > 0:
            for i in unknowns:
                ...
                if name:
                    folder_path = 'face'
                    path = osp.abspath(folder_path)
                    id = self.faces_database.dump_faces(crop_image, face_identities[i].descriptor, name, path)
                    face_identities[i].id = id
            for a in all_list:
                # This check is preventing asking to save half-images in the boundary of images
                if rois[a].position[0] == 0.0 or rois[a].position[1] == 0.0 or \
                    (rois[a].position[0] + rois[a].size[0] > orig_image.shape[1]) or \
                    (rois[a].position[1] + rois[a].size[1] > orig_image.shape[0]):
                    continue
                crop_image = crop(orig_image, rois[a])
                name = self.faces_database.ask_to_save(crop_image)
                if name:
                    folder_path2 = 'fa'
                    path2 = osp.abspath(folder_path2)
                    self.faces_database.dump_faces(crop_image, face_identities[a].descriptor, name, path2)
                    # face_identities[a].id = id
...
def draw_detections(frame, frame_processor, detections, output_transform):
    ...
    for roi, landmarks, identity in zip(*detections):
        ...
        # print('-----identity')
        # # print(identity.confidence_threshold)
        # print('-----identity')
        xmin = max(roi.position[0], 0)
        ymin = max(roi.position[1], 0)
        xmax = min(roi.position[0] + roi.size[0], size[1])
        ymax = min(roi.position[1] + roi.size[1], size[0])
        ...
        # img_reg = frame_copy[ymin:ymax, xmin:xmax]
        
        # gender_blob = cv2.dnn.blobFromImage(img_reg, 1.0, (227, 227), MODEL_MEAN_VALUES, swapRB=False)
        # gen_net.setInput(gender_blob)
        # gender_preds = gen_net.forward()
        # gender_label = gender_labels[gender_preds[0].argmax()]


        # # Crear blob para la predicción de edad
        # age_blob = cv2.dnn.blobFromImage(img_reg, 1.0, (227, 227), MODEL_MEAN_VALUES, swapRB=False)
...
def center_crop(frame, crop_size):
    print('asdjashdkashdkashdahdkjahkjdhkas')
    fh, fw, _ = frame.shape
...
def main():
    ...
    video_url = 'rtsp://admin:1234qwer@192.168.0.249:554/doc/page/preview.asp'
    ...
    while True:
        ...
        if input_crop is not None:
            # print('siuuuuuuuuuuuuuuuuuuuuuuuuu')
...
def extract_bounding_boxes(yolo_results, frame):
    ...
    for boxes in yolo_results:
        face_boxes = boxes.boxes.xyxy
        for box_info in face_boxes:
            x1, y1, x2, y2 = map(int, box_info[:4])

            ro = FaceDetector.Result([0,0,0,x1, y1, x2 - x1, y2 - y1])
